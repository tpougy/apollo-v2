...
`--entidade-id`, when supplied, links the ticket to that entidade via the
`entidadeTickets` link (forward label `entidade`, on `tickets`). The parent
id is validated with `get_entity` before any write — InstantDB does not
check link targets exist, so an unchecked link would happily write a
dangling reference.
"""
...
def _resolve_entidade_link(entidade_id: str | None) -> dict[str, str] | None:
    ...
    look like a normal ticket until someone tries to read the link back.
    """
...
@click.option(
    "--entidade-id",
    default=None,
    help="Optional id of an `entidade` to link this ticket to. Must already exist.",
)
def criar(
    titulo: str,
    corpo: str,
...
@click.option(
    "--entidade-id",
    default=None,
    help="New id of an `entidade` to link this ticket to. Must already exist.",
)
def editar(
    eid: str,
    titulo: str | None,
...
@group.command()
@click.option("--entidade-id", default=None, help="Filter to tickets of exactly this entidade.")
...
def listar(entidade_id: str | None, status: str | None, limit: int | None) -> None:
    """List tickets visible to the authenticated session."""
    where = drop_none({"status": status, "entidade.id": entidade_id})
